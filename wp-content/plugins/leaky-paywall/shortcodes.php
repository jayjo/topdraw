...
if ( !function_exists( 'do_leaky_paywall_login' ) ) {
	...
	function do_leaky_paywall_login( $atts ) {
		...
		if ( 'passwordless' === $settings['login_method'] ) {
			...
			$results .= '<input type="submit" id="leaky-paywall-submit-buttom" name="submit-leaky-login" kind="Forgot Login Email" location="" value="' . __( 'Send Login Email', 'issuem-leaky-paywall' ) . '" />';
			$results .= '</form>';
		...
		if ( is_user_logged_in() ) {
			...
			foreach( $sites as $site ) {
				...
				if ( !empty( $plan ) && 'Canceled' !== $plan && 'Never' !== $expires ) {
					$expires = sprintf( __( 'Renews, %s', 'issuem-leaky-paywall' ), $expires );
				}
			...
			$results .= '<p class="submit"><input type="submit" id="submit" class="button button-primary" kind="Save Profile" location="Profile" value="' . __( 'Save Profile Changes', 'issuem-leaky-paywall' ) . '"  /></p>';
			$results .= '</form>';
					...
					if ( 'subscription' === $expires ) {
						...
						switch( $payment_gateway ) {
							...
							case 'stripe':
								...
								$payment_form .= '<p class="submit"><input type="submit" id="submit" class="button button-primary" location="Payment Form" kind="Update Credit Card" value="' . __( 'Update Credit Card Information', 'issuem-leaky-paywall' ) . '"  /></p>';
								$payment_form .= '</form>';
								break;
	...
	leaky_paywall_show_error_messages( 'register' );
		...
		<form action="" method="POST" name="payment-form" id="leaky-paywall-payment-form" class="leaky-paywall-payment-form">
		  ...
		  <div class="leaky-paywall-checkout-button">
		  	<button id="leaky-paywall-submit" type="submit" class="submit" location="Checkout" kind="Subscription"><?php printf( __( 'Subscribe', 'leaky-paywall' ) ); ?></button>
		  </div>
		</form>
